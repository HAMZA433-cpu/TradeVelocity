...
const MacroSentiment = () => {
    ...
    const fetchMacroIndicators = async () => {
        try {
            const response = await axios.get('${API_BASE_URL}/api/macro/indicators');
    ...
    const fetchSentimentScore = async () => {
        try {
            const response = await axios.get('${API_BASE_URL}/api/macro/sentiment-score');
            if (response.data.success) {
    ...
    const fetchCorrelation = async (period = '6mo') => {
        try {
            const response = await axios.get(`${API_BASE_URL}/api/macro/correlation/${period}`);
            if (response.data.success) {
    ...
    const fetchHistoricalData = async (ticker, period) => {
        try {
            const response = await axios.get(`${API_BASE_URL}/api/macro/historical/${ticker}/${period}`);
            if (response.data.success) {
