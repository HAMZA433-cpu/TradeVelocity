...
const TransactionManagement = () => {
    ...
    const fetchTransactions = async () => {
        try {
            const response = await axios.get(
                `${API_BASE_URL}/api/admin/transactions?status=${filter === 'all' ? '' : filter}`,
    ...
    const fetchPending = async () => {
        try {
            const response = await axios.get(
                '${API_BASE_URL}/api/admin/transactions/pending',
                { headers: { 'X-User-ID': user.id } }
            );
    ...
    const handleApprove = async (transactionId) => {
        try {
            await axios.put(
                `${API_BASE_URL}/api/admin/transactions/${transactionId}/approve`,
                {},
                { headers: { 'X-User-ID': user.id } }
            );
            fetchTransactions();
    ...
    const handleReject = async (transactionId) => {
        ...
        try {
            await axios.put(
                `${API_BASE_URL}/api/admin/transactions/${transactionId}/reject`,
                { reason },
                { headers: { 'X-User-ID': user.id } }
            );
            fetchTransactions();
