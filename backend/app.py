...
# Autoriser toutes les origines pour le moment (à sécuriser plus tard en production)
CORS(app, 
     resources={r"/*": {"origins": "*"}},
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///tradesense.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Flask Secret Key (required for sessions and OAuth)
...
from modules.google_oauth import google_oauth_bp, oauth  # Import Google OAuth
...
app.register_blueprint(google_oauth_bp, url_prefix='/auth')  # Google OAuth routes
...
@app.route('/')
def home():
    return {"message": "TradeVelocity Backend is Running"}
...
if __name__ == '__main__':
    print("🚀 TradeVelocity Backend Starting...")
    print("📡 WebSocket server enabled")
    port = int(os.getenv('PORT', 5000))  # Render fournit PORT
    print(f"🌐 Running on port {port}")
    # Pour production avec gunicorn, on n'utilise pas debug=True
